...
Computing $y = A x$ when $A$ is encoded as in COO format simply
consist in executing for every nonzero $j$: $y[rowA[j]] +=
x[colA[j]] * valA[j]$.
...
\begin{table*}
\caption{Performance of the SVR SpMV model on RMAT matrices.\todo{columns should be ordered like in the previous tables. Any chance we will get the models for linear and benchmark based?}}
