...
32\%. The main reason behind this this error is the density of the
matrices. As we can see All R-MAT matrices are highly dense compare to
the real-world test matrices. We build up our matrices by varying
non-zero per row by the multiply of $2(1,2,4, 8, 16, 32, 64, 128$, so
when the test cases are more dense the angle between two candidate row
for the linear model become large.
