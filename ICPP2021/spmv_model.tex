...
\begin{table}[htb]
\caption{Overall SpMV on Random 2D Partitioning(on Skylake).}
\label{tab:overall-spmv-2d-single}
\centering
\begin{tabular}[c]{| l | c | c | c | c | c |}
\hline
...
\subsubsection{CSR}
\begin{table}[htb]
\caption{Overall SpMV on Global 1D-Row Partitioning(on Skylake).}
\label{tab:overall-spmv-gk-single}
\centering
\begin{tabular}[c]{| l | c | c | c | c | c |}
\hline
\multirow{2}{*}{Matrices} & \multirow{2}{*}{Nodes} & \multirow{2}{*}{Processes} & \multicolumn{2}{| c |}{Time(s)} & \multirow{2}{*}{Error\%} \\ \cline{4-5}
